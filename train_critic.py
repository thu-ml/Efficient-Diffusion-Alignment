...
from BDiffusion import IQL_policy
...
def train_critic(args, score_model, data_loader, start_epoch=0):
    ...
    for epoch in tqdm_epoch:
        ...
        if args.save_model and ((epoch % save_interval == (save_interval - 1)) or epoch==0):
            # if not os.path.exists(os.path.join("./EDA_model_factory", "{}-baseline-seed{}".format(args.env, args.seed))):
            #     os.makedirs(os.path.join("./EDA_model_factory", "{}-baseline-seed{}".format(args.env, args.seed)))
            # torch.save(score_model.q[0].state_dict(), os.path.join("./EDA_model_factory", "{}-baseline-seed{}".format(args.env, args.seed), "critic_ckpt{}.pth".format(epoch+1)))
            torch.save(score_model.state_dict(), os.path.join("./EDA_model_factory", str(args.expid), "critic_ckpt{}.pth".format(epoch+1)))

def critic(args):
    for dir in ["./EDA_model_factory"]:
        if not os.path.exists(dir):
            os.makedirs(dir)
    if not os.path.exists(os.path.join("./EDA_model_factory", str(args.expid))):
        os.makedirs(os.path.join("./EDA_model_factory", str(args.expid)))
    run = wandb.init(project="EDA_model_factory", name=str(args.expid))
    ...
    score_model= IQL_policy(input_dim=state_dim+action_dim, output_dim=action_dim, args=args).to(args.device)
